...
class NatureCNN(nn.Module):
    def __init__(self, sample_obs):
        super().__init__()
        # print("Initializing NatureCNN with observation keys:", sample_obs.keys()) # for debugging
        ...
        if "rgbd" in sample_obs:
            # print(f"RGBD shape: {sample_obs['rgbd'].shape}") # for debugging
            # split last dimension (first 3 channels for RGB, last channel for depth)
            # RGBD shape is [B, H, W, C] where C = 16 (RGB + depth features)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.reconfiguration_freq is not None and iteration % args.reconfiguration_freq == 0 and args.partial_reset == False:
        ...
        if iteration % args.eval_freq == 1:
            ...
            if args.num_eval_envs > 0 and hasattr(eval_envs.unwrapped, 'reconfigure') and args.eval_partial_reset == False and args.eval_reconfiguration_freq is not None:
